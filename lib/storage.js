  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      if (!db.objectStoreNames.contains('bookmarks')) {
        ...
        bm.createIndex('category', 'category', { unique: false });
      } else {
        // v5: add category index to existing bookmarks store
        const bm = e.target.transaction.objectStore('bookmarks');
        if (!bm.indexNames.contains('category')) {
          bm.createIndex('category', 'category', { unique: false });
        }
      }
    };
...
const Storage = {
  ...
  async getBookmarks(options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      req.onsuccess = () => {
        ...
        if (options.category) {
          results = results.filter(bm => bm.category === options.category);
        }
        if (options.search) {
          const q = options.search.toLowerCase();
  ...
  async clearAll() {
    const db = await openDB();
    const stores = ['visits', 'settings', 'concepts', 'relations', 'bookmarks'];
